...
import { testQuestionnaireGenerated, testQuestionnaireGenerationError } from 'containers/Home/actions';
...
export function* generateTestQuestionnaire() {
  ...
  if (!data.err) {
    yield put(questionnaireListLoaded(data.questionnaire));
  } else {
    yield put(questionnaireListLoadingError(data.err));
  }
}
